...
const VIDEO_EXT = { "video/mp4": "mp4", "video/quicktime": "mov", "video/webm": "webm", "video/x-m4v": "m4v" };

export function isAllowedPhotoType(contentType) {
  return ALLOWED_TYPES.has((contentType || "").toLowerCase());
}

export function isAllowedVideoType(contentType) {
  return ALLOWED_VIDEO_TYPES.has((contentType || "").toLowerCase());
}

async function upload(objectPath, buffer, contentType = "image/jpeg") {
...
// Event video: uploaded as-is (no transcoding) to Supabase Storage.
export async function saveEventVideo(eventId, buffer, contentType) {
  const ext = VIDEO_EXT[(contentType || "").toLowerCase()] || "mp4";
  return upload(`events/event-${eventId}-video.${ext}`, buffer, contentType || "video/mp4");
}
