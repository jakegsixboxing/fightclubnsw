...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS results (
    ...
    method TEXT,
    fighter_of_night INTEGER NOT NULL DEFAULT 0,
...
try { db.exec("ALTER TABLE matches ADD COLUMN result_recorded INTEGER NOT NULL DEFAULT 0"); } catch (e) {}
try { db.exec("ALTER TABLE matches ADD COLUMN winner_fighter_id INTEGER"); } catch (e) {}
try { db.exec("ALTER TABLE matches ADD COLUMN method TEXT"); } catch (e) {}
...
// Record the result of a confirmed match and award points to both fighters.
export function recordMatchResult({ matchId, winnerFighterId, method, fotnFighterId }) {
  const m = db.prepare("SELECT * FROM matches WHERE id = ?").get(matchId);
  if (!m || m.status !== "confirmed") return false;
  if (m.result_recorded) {
    // wipe previous result rows for this match so we can re-record
    db.prepare("DELETE FROM results WHERE match_id = ?").run(matchId);
  }
  const ev = m.event_id ? db.prepare("SELECT event_date FROM events WHERE id = ?").get(m.event_id) : null;
  const season = seasonOf(ev ? ev.event_date : null) || "2026";
  ...
  for (const fid of ids) {
    ...
    db.prepare(
      `INSERT INTO results (fighter_id, match_id, event_id, outcome, method, fighter_of_night, points, season)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(fid, matchId, m.event_id || null, outcome, method || null, fotn ? 1 : 0, points, season);
  }

  db.prepare(
    "UPDATE matches SET result_recorded = 1, winner_fighter_id = ?, method = ?, updated_at = datetime('now') WHERE id = ?"
  ).run(winnerFighterId || null, method || null, matchId);
  return true;
}
  ...
  return db
    .prepare(
      `SELECT fighters.*, weight_divisions.label AS division_label, weight_divisions.boxer_type AS division_type,
              ...
              COUNT(results.id) AS fights,
              SUM(CASE WHEN results.outcome='win' THEN 1 ELSE 0 END) AS wins_season,
              SUM(results.fighter_of_night) AS fotn_count
       FROM fighters
       LEFT JOIN results ON results.fighter_id = fighters.id AND results.season = ?
       LEFT JOIN weight_divisions ON weight_divisions.id = fighters.weight_division_id
       GROUP BY fighters.id
       HAVING points > 0
       ORDER BY points DESC, wins_season DESC, fighters.full_name ASC`
    )
    .all(s);
}

export function getFighterPoints(fighterId, season) {
  const r = db
    .prepare("SELECT COALESCE(SUM(points),0) AS points, COUNT(*) AS fights FROM results WHERE fighter_id = ? AND season = ?")
    .get(fighterId, season || "2026");
  return r || { points: 0, fights: 0 };
}

export function getConfirmedMatchesAll() {
  return db
    .prepare(`${MATCH_SELECT} WHERE m.status = 'confirmed' ORDER BY ev.event_date ASC, m.updated_at DESC`)
    .all();
}
