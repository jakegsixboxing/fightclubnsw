...
export async function setEventVideo(eventId, url) {
  await run(supabase.from("pf_events").update({ video_path: url }).eq("id", eventId));
}
...
// Matching Pool: every fighter currently nominated to at least one event,
// with the list of event titles they've put their hand up for. This is what
// powers the "Matching Pool" tab — the moment a fighter nominates, they show
// up here for the matchmaker to pair.
export async function getMatchingPoolFighters() {
  const nomRows = await run(supabase.from("pf_nominations").select("fighter_id, event_id"));
  if (!nomRows.length) return [];
  const fighterIds = [...new Set(nomRows.map((r) => r.fighter_id))];
  const eventIds = [...new Set(nomRows.map((r) => r.event_id))];

  const [fighters, events] = await Promise.all([
    run(supabase.from("pf_fighters_flat").select("*").in("id", fighterIds)),
    run(supabase.from("pf_events").select("id, title").in("id", eventIds)),
  ]);
  const eventTitleById = new Map(events.map((e) => [e.id, e.title]));
  const eventsByFighter = new Map();
  for (const r of nomRows) {
    const list = eventsByFighter.get(r.fighter_id) || [];
    const title = eventTitleById.get(r.event_id);
    if (title) list.push(title);
    eventsByFighter.set(r.fighter_id, list);
  }

  return fighters
    .map((f) => ({ ...f, nominated_events: eventsByFighter.get(f.id) || [] }))
    .sort((a, b) => {
      if ((a.division_type || "") !== (b.division_type || "")) return (a.division_type || "").localeCompare(b.division_type || "");
      if ((a.division_sort ?? 0) !== (b.division_sort ?? 0)) return (a.division_sort ?? 0) - (b.division_sort ?? 0);
