...
export function registerPage({ fighter = {}, weightDivisionsByType, error, isEdit }) {
  ...
  const steps = [
    { title: "You", sub: "Let's start with a clear photo and your name." },
    { title: "Your Gym", sub: "Who you train with, and where." },
    { title: "Division", sub: "Amateur or pro, and your weight class." },
    { title: "Record", sub: "Your honest record — wins, losses, KOs." },
    { title: "Titles", sub: "Any titles held, past or present." },
    { title: "Availability", sub: "Are you open to fight offers right now?" },
  ];
  const dots = steps.map((_, i) => `<span class="wizard-dot" data-wizard-dot="${i + 1}"></span>`).join("");
  ...
  <section class="section">
    <div class="wrap">
      <div class="card form-card wide">
        ...
        ${error ? `<div class="flash flash-error">${escapeHtml(error)}</div>` : ""}
        <form method="POST" action="/register" enctype="multipart/form-data" data-wizard novalidate>
          <div class="wizard-bar">
            <div class="wizard-dots">${dots}</div>
          </div>
          <div class="wizard-heading">
            <h3 data-wizard-title>${steps[0].title}</h3>
            <p data-wizard-sub>${steps[0].sub}</p>
          </div>

          <div class="wizard-step" data-step="1" data-title="${escapeHtml(steps[0].title)}" data-sub="${escapeHtml(steps[0].sub)}">
            <div class="field">
              <label for="full_name">Full Name</label>
              <input type="text" id="full_name" name="full_name" value="${escapeHtml(fighter.full_name || "")}" required>
            </div>
            <div class="field">
              <label for="photo">Photo ${isEdit ? "(upload a new one to replace your current photo)" : ""}</label>
              <input type="file" id="photo" name="photo" accept="image/png,image/jpeg,image/webp" ${isEdit ? "" : "required"}>
              <p class="help-text">A clear, front-facing photo. This is what everyone sees on your profile and fight card.</p>
          ...
          <div class="wizard-step" data-step="2" data-title="${escapeHtml(steps[1].title)}" data-sub="${escapeHtml(steps[1].sub)}" hidden>
            <div class="field">
          ...
          <div class="wizard-step" data-step="3" data-title="${escapeHtml(steps[2].title)}" data-sub="${escapeHtml(steps[2].sub)}" hidden>
            <div class="field">
          ...
          <div class="wizard-step" data-step="4" data-title="${escapeHtml(steps[3].title)}" data-sub="${escapeHtml(steps[3].sub)}" hidden>
          ...
          <div class="wizard-step" data-step="5" data-title="${escapeHtml(steps[4].title)}" data-sub="${escapeHtml(steps[4].sub)}" hidden>
            <div class="field">
          ...
          <div class="wizard-step" data-step="6" data-title="${escapeHtml(steps[5].title)}" data-sub="${escapeHtml(steps[5].sub)}" hidden>
            <div class="field">
          ...
          <div class="wizard-nav">
            <button type="button" class="btn btn-outline" data-wizard-back hidden>Back</button>
            <button type="button" class="btn btn-gold" data-wizard-next>Next →</button>
            <button type="submit" class="btn btn-gold" data-wizard-submit hidden>${isEdit ? "Save Changes" : "Create My Profile"}</button>
          </div>
        </form>
      </div>
    </div>
